...
func SetupGated(mgr ctrl.Manager, o tjcontroller.Options) error {
	o.Options.Gate.Register(func() {
		if err := Setup(mgr, o); err != nil {
			mgr.GetLogger().Error(err, "unable to setup reconciler", "gvk", v1beta1.GroupRoleManagementPolicy_GroupVersionKind.String())
		}
	}, v1beta1.GroupRoleManagementPolicy_GroupVersionKind)
	return nil
}

// Setup adds a controller that reconciles GroupRoleManagementPolicy managed resources.
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	name := managed.ControllerName(v1beta1.GroupRoleManagementPolicy_GroupVersionKind.String())
	var initializers managed.InitializerChain
	eventHandler := handler.NewEventHandler(handler.WithLogger(o.Logger.WithValues("gvk", v1beta1.GroupRoleManagementPolicy_GroupVersionKind)))
	ac := tjcontroller.NewAPICallbacks(mgr, xpresource.ManagedKind(v1beta1.GroupRoleManagementPolicy_GroupVersionKind), tjcontroller.WithEventHandler(eventHandler), tjcontroller.WithStatusUpdates(false))
	rl := reconciliationpolicy.NewExponentialFailureRateLimiter(time.Second, 60*time.Second)
	opts := []managed.ReconcilerOption{
		managed.WithExternalConnecter(
			tjcontroller.NewTerraformPluginSDKAsyncConnector(mgr.GetClient(), o.OperationTrackerStore, o.SetupFn, o.Provider.Resources["azuread_group_role_management_policy"],
				...
				tjcontroller.WithTerraformPluginSDKAsyncMetricRecorder(metrics.NewMetricRecorder(v1beta1.GroupRoleManagementPolicy_GroupVersionKind, mgr, o.PollInterval)),
	...
	// register webhooks for the kind v1beta1.GroupRoleManagementPolicy
	// if they're enabled.
	if o.StartWebhooks {
	...
	if o.MetricOptions != nil && o.MetricOptions.MRStateMetrics != nil {
		stateMetricsRecorder := statemetrics.NewMRStateRecorder(
			mgr.GetClient(), o.Logger, o.MetricOptions.MRStateMetrics, &v1beta1.GroupRoleManagementPolicyList{}, o.MetricOptions.PollStateMetricInterval,
		)
		if err := mgr.Add(stateMetricsRecorder); err != nil {
			return errors.Wrap(err, "cannot register MR state metrics recorder for kind v1beta1.GroupRoleManagementPolicyList")
		}
	}
	...
	r := managed.NewReconciler(mgr, xpresource.ManagedKind(v1beta1.GroupRoleManagementPolicy_GroupVersionKind), opts...)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&v1beta1.GroupRoleManagementPolicy{}, eventHandler).
